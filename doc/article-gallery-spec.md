...
Present articles as a horizontally snapping discovery gallery. Each card represents one article and combines a stable editorial image, a short canonical title, a short description, and the article's demonstrations as a segmented control attached directly to the bottom edge. The card body opens the article; the demonstration segments apply or remove presets. Articles without demonstrations remain complete knowledge cards and have no disabled or placeholder action area.

Keep the live camera as the result surface and place the gallery before the settings in the same vertical scroll container. A compact passive index below the gallery communicates article position and which articles have active presets. The index uses shape and thickness in addition to color.
...
4. As a user, I want a card's image, title, and description to open its article, so that the article has a large and obvious touch target.
5. As a user, I want an information icon on the card, so that the card's educational purpose is explicit.
...
28. As a user, I want multi-form cards to use a comparison image and shared description, so that distinct visual forms are understandable without duplicating an article.
...
47. As a content editor, I want a localized article summary, so that each card can explain the perceptual experience in at most two lines.
48. As a content editor, I want one 16:9 article image reference, so that clients can build stable cards without unpredictable cropping.
49. As a catalog maintainer, I want each preset to belong unambiguously to one article, so that preset provenance can always open the correct knowledge source.
50. As an iOS or Web implementer, I want presentation-neutral catalog metadata, so that later clients can adopt the design without Android concepts in the shared contract.
...
- The shared article contract gains localized `summary` and `image` metadata. They may remain optional during content migration; Android must render a useful card when either is absent. They should become required for indexed articles before the gallery is considered content-complete.
...
- Existing article titles become short canonical titles. Parenthetical aliases and explanatory wording move to the summary or article introduction rather than a second title field.
...
- A gallery card represents an article, not a preset. Its demonstrations are rendered as one attached segmented control. The card body and information affordance open the article; only demonstration segments mutate simulation state.
- An article with no demonstrations has no action footer and no unavailable-state messaging. Its content area fills the same total card height.
...
- The gallery, passive index, and settings share one vertical scroll container. The gallery is not sticky.
...
- The highest shared test seam is the serialized catalog contract. Contract tests should verify exact article order, index completeness, unique identifiers, optional metadata serialization, short-label localization, demonstration references, and unique preset ownership.
...
- Creating final gallery images and final short descriptions. The schema and fallbacks are included, but content production may follow separately.
...
- The card summary should describe the perceptual experience rather than causes or clinical definition, fit in at most two lines, and avoid repeating the title.
- The gallery image and summary are stable editorial context. The live camera remains the authoritative visualization of the currently active simulation.
