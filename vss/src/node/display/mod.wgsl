...
fn fs_main(in: VertexOutput) -> FragmentOutput {
    ...
    switch (uniforms.base_image) {
        case 0:{ // simulated image
            color = textureSampleLevel(in_color_t, in_color_s, in.tex, 0.0).rgb;
        }
        case 1:{ // original image
            color = textureSampleLevel(in_original_t, in_original_s, in.tex, 0.0).rgb;
        }
        case 2:{
            color = RetinalGanglion(in.tex);
        }
        case 3:{
            color = textureSampleLevel(in_color_t, in_color_s, in.tex, 0.0).rgb;
        }
    ...
    if (uniforms.mix_type != 0){ // -- only calculate colormaps if they may be used
        ...
        switch (uniforms.combination_function){
            case 0:{ // Length of RGB error
                let e_rgb_2 = pow(textureSampleLevel(in_color_change_t, in_color_change_s, in.tex, 0.0).rgb, vec3<f32>(2.0));
                value = sqrt( e_rgb_2.r + e_rgb_2.g + e_rgb_2.b);
            }
            case 1:{ // Length of XY error
                let e_xy_2 = pow(textureSampleLevel(in_deflection_t, in_deflection_s, in.tex, 0.0).xy / max_e_xy, vec2<f32>(2.0)) ; 
                value = sqrt( e_xy_2.x + e_xy_2.y );
            }
            case 2:{ // Length of RGBXY error
                let e_5_rgb_2 = pow(textureSampleLevel(in_color_change_t, in_color_change_s, in.tex, 0.0).rgb, vec3<f32>(2.0));
                let e_5_xy_2 = pow(textureSampleLevel(in_deflection_t, in_deflection_s, in.tex, 0.0).xy / max_e_xy, vec2<f32>(2.0));
                value = sqrt( e_5_rgb_2.r + e_5_rgb_2.g + e_5_rgb_2.b + e_5_xy_2.x + e_5_xy_2.y );
            }
            case 3:{ // Length of RGB uncertainty
                let u_rgb = textureSampleLevel(in_color_uncertainty_t, in_color_uncertainty_s, in.tex, 0.0).rgb;
                value = sqrt( u_rgb.r + u_rgb.g + u_rgb.b);
            }
            case 4:{ // Length of XY uncertainty
                let u_xy = textureSampleLevel(in_deflection_t, in_deflection_s, in.tex, 0.0).ba / max_u_2_xy;
                value = sqrt( u_xy.x + u_xy.y );
            }
            case 5:{ // Length of RGBXY uncertainty
                let u_5_rgb = textureSampleLevel(in_color_uncertainty_t, in_color_uncertainty_s, in.tex, 0.0).rgb;
                let u_5_xy = textureSampleLevel(in_deflection_t, in_deflection_s, in.tex, 0.0).ba / max_u_2_xy;
                value = sqrt(u_5_rgb.r + u_5_rgb.g + u_5_rgb.b + u_5_xy.x + u_5_xy.y );
            }
            case 6:{ // GenvarRGB
                let cvm_var = textureSampleLevel(in_color_uncertainty_t, in_color_uncertainty_s, in.tex, 0.0).rgb;
                let cvm_covar = textureSampleLevel(in_covariances_t, in_covariances_s, in.tex, 0.0).rgb;
                let cvm = mat3x3<f32>(
