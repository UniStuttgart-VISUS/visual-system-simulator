...
vec4 blur_sd(in vec2 fragCoord, in sampler2D tex, float blur_scale, vec2 resolution, inout vec3 var, in sampler2D vartex) {
    // declare stuff
...
vec2 getPerceivedBrightness_sd(in vec3 color, in vec3 var_in) {
    // The constants adjust the contribution of each color to the perceived brightness, according to the amount of reception in the eye. See https://www.w3.org/TR/AERT/#color-contrast
