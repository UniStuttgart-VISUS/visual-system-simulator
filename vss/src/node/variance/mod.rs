...
impl VarianceMeasure {
    pub fn new(surface: &Surface) -> Self {
        ...
        let (sources_bind_group_layout, sources_bind_group) =
            create_color_sources_bind_group(device, queue, "Variance");

        let original_tex =
            placeholder_texture(device, queue, Some("VarianceNode s_original")).unwrap();
        ...
        let color_measurement_pipeline = create_render_pipeline(
            device,
            &[&shader, &shader],
            &["vs_main", "fs_color_measurement"],
            &[
            ...
            None,
            Some("VarianceNode Color Measurement Render Pipeline"),
        );
        ...
        let metrics_ab_pipeline = create_render_pipeline(
            ...
            None,
            Some("VarianceNode Metrics AB Render Pipeline"),
        );
        ...
        let metrics_cd_pipeline = create_render_pipeline(
            ...
            None,
            Some("VarianceNode Metrics CD Render Pipeline"),
        );
    ...
    fn measure_variance(&mut self, surface: &Surface) -> (f32, f32) {
        ...
        if self.uniforms.data.variance_metric == 6 {
            assert!(
                self.uniforms.data.show_variance < 3,
                "Histogram can only be calculated with variance Type <Before> or <After>"
            );
            assert!(
                self.uniforms.data.color_space == 2,
                "Histogram can only be calculated with color Type <ITP>"
            );
            ...
            for chunk in padded_buffer.chunks(self.buffer_dimensions.padded_bytes_per_row) {
                let pixels: &[f32] = unsafe {
                    std::slice::from_raw_parts(
                        ...
                        self.buffer_dimensions.unpadded_bytes_per_row / 4,
                    )
                };
                for i in (0..self.buffer_dimensions.unpadded_bytes_per_row / 4).step_by(4) {
                    let pixel_key = (((pixels[i] * 255.0) as u32) << 16)
                        | (((pixels[i + 1] * 255.0) as u32) << 8)
                        | ((pixels[i + 2] * 255.0) as u32);
                    color_map.entry(pixel_key).or_insert([
                        pixels[i],
                        pixels[i + 1],
                        pixels[i + 2],
                        0.0,
                    ])[3] += 1.0 / (width as f32 * height as f32);
                }
            }
            ...
            for (index, (_color_key, color)) in color_map.iter().enumerate() {
                for (index_inner, (_color_key_inner, color_inner)) in color_map.iter().enumerate() {
                    ...
                    let diff = (
                        color_inner[0] - color[0],
                        color_inner[1] - color[1],
                        color_inner[2] - color[2],
                    );
                    let length =
                        (diff.0 * diff.0 + diff.1 * diff.1 * 0.5 + diff.2 * diff.2).sqrt() as f64;
            ...
            assert!(
                (self.uniforms.data.variance_metric != 5) || (self.uniforms.data.color_space == 1),
                "Michelson Contrast can only be calculated with color Type <LAB>"
            );
            // sum up variance
            ...
            (
                sum_variance,
                sum_variance
                    / ((self.buffer_dimensions.width * self.buffer_dimensions.height) as f32),
            )
        }
    }
}
...
impl Node for VarianceMeasure {
    ...
    fn negociate_slots(
        ...
        self.buffer_dimensions = BufferDimensions::new(
            self.uniforms.data.resolution[0] as usize,
            self.uniforms.data.resolution[1] as usize,
            size_of::<[f32; 4]>(),
        );
        let download_buffer = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("Download Node Buffer"),
            size: (self.buffer_dimensions.padded_bytes_per_row * self.buffer_dimensions.height)
                as u64,
            usage: wgpu::BufferUsages::MAP_READ | wgpu::BufferUsages::COPY_DST,
            mapped_at_creation: false,
        });
    ...
    fn render(
...

        {
            let mut render_pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                label: Some("Variance color_measurement_pass"),
                color_attachments: &[
                    screen
                        .unwrap_or(&self.targets.rt_color)
        ...
        if Instant::now().duration_since(self.last_info).as_secs_f32() >= 1.0
            && self.uniforms.data.show_variance > 0
        {
    ...
    fn post_render(&mut self, surface: &Surface) {
        if self.should_download {
            ...
            if self.uniforms.data.variance_metric == 6 {
                println!(
                    "Total amount of colors: {:?}\t Avg histogram variance: {:?}",
                    sum, avg
                );
            } else {
                println!(
                    "Variance sum: {:?}\t Avg variance per pixel: {:?}",
                    sum, avg
                );
            }
        }
